...
bool RendererOGL::initialize(Window& windowP)
{
	...
	// Create G-buffer
	gBuffer = new GBuffer();
	int width = static_cast<int>(WINDOW_WIDTH);
	int height = static_cast<int>(WINDOW_HEIGHT);
	if (!gBuffer->create(width, height))
	{
		Log::error(LogCategory::Video, "Failed to create G-buffer.");
		return false;
	}
...
{
	draw3dScene(mirrorBuffer, mirrorView, projection, 0.25f, true);
	draw3dScene(gBuffer->getBufferID(), view, projection, 1.0f, false);
...
void RendererOGL::draw3dScene(unsigned int frameBuffer, const Matrix4& view, const Matrix4& proj, float viewportScale, bool lit)
{
...
void RendererOGL::initializeGBuffer()
{
	Shader& gGlobalShader = Assets::getShader("GGlobal");
	// For the GBuffer, we need to associate each sampler with an index
	gGlobalShader.use();
	gGlobalShader.setInteger("uGDiffuse", 0);
	gGlobalShader.setInteger("uGNormal", 1);
	gGlobalShader.setInteger("uGWorldPos", 2);
	// The view projection is just the sprite one
	gGlobalShader.setMatrix4("uViewProj", spriteViewProj);
	// The world transform scales to the screen and flips y
	Matrix4 gbufferWorld = Matrix4::createScale(WINDOW_WIDTH, -WINDOW_HEIGHT, 1.0f);
	gGlobalShader.setMatrix4("uWorldTransform", gbufferWorld);
}

void RendererOGL::drawFromGBuffer()
{
	...
	setLightUniforms(gGlobalShader);
	// Draw the triangles
	...
	Shader& pointLightShader = Assets::getShader("GPointLight");
	pointLightShader.use();
	Mesh& pointLightMesh = Assets::getMesh("Mesh_PointLight");
	pointLightMesh.getVertexArray()->setActive();
	// Set the view-projeciton matrix
	pointLightShader.setMatrix4("uViewProj", view * projection);
	...
	{
		p->draw(pointLightShader, pointLightMesh);
	}
}

void RendererOGL::addPointLight(PointLightComponent* light)
{
	pointLights.emplace_back(light);
}

void RendererOGL::removePointLight(PointLightComponent* light)
{
	auto iter = std::find(begin(pointLights), end(pointLights), light);
	if (iter != end(pointLights))
	{
		pointLights.erase(iter);
	}
}

void RendererOGL::initializeGPointLight()
{
	Shader& pointlightShader = Assets::getShader("GPointLight");
	pointlightShader.use();
	pointlightShader.setInteger("uGDiffuse", 0);
	pointlightShader.setInteger("uGNormal", 1);
	pointlightShader.setInteger("uGWorldPos", 2);
	pointlightShader.setVector2f("uScreenDimensions", Vector2(WINDOW_WIDTH, WINDOW_HEIGHT));
}
